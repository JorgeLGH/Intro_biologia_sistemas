---
title: "Motivos en redes de regulación trancripcional"
runningheader: "FFL-C1" # only for pdf output
...
![Feed Forward Loops](05_Images/Captura de Pantalla 2022-03-16 a la(s) 17.22.09.png)

## Motivos 

![](05_Images/Captura de Pantalla 2022-03-16 a la(s) 17.22.15.png)

### Coherent 1 Feed Forward Loop
![](05_Images/Captura de Pantalla 2022-03-16 a la(s) 17.22.22.png)


![](05_Images/Captura de Pantalla 2022-03-16 a la(s) 17.22.28.png)
...
```{r}
atractores<-getAttractors(FFLC1)
...
## Continuo con función escalón

Las ecuaciones considerando la aproximación de la regulación como función escalón.

$$\frac{dY}{dt}=\beta_Y\theta(x>K_{XY}) -\alpha_{Y}Y$$
$$\frac{dZ}{dt}=\beta_{Z}\theta(x>K_{XZ})\theta(Y>K_{YZ})-\alpha_Z Z$$
...
```{r}
library(deSolve)
FFL_C1 <- function(t, state, parameters) {
with(as.list(c(state, parameters)), { 
       # Acá van definidas las ecuaciones
       dX<- 0
       dY<- beta_y*mi_funcion_escalon(X,1/2)-alfa_y*Y
       dZ <- beta_Z *mi_funcion_escalon(X,1/2)*mi_funcion_escalon(Y, beta_y/(2*alfa_y))-alfa_z*Z
        list(c( dX,dY, dZ))
      })
}
# Acá le damos valores a los parámetros
parameters <- c(beta_y =3, beta_Z = 4, alfa_y =1 ,alfa_z=2)
...
out <- ode(y = state, times = times, func = FFL_C1 , parms = parameters)
...
## Modelo continuo con funciones de Hill

$$\frac{dY}{dt}=\beta_y \frac{x^n}{k^n +x^n}-\alpha_y Y $$

$$\frac{dZ}{dt}=\beta_z\beta_y (\frac{y^n}{k^n +y^n})( \frac{x^n}{k^n +x^n})-\alpha_z Z $$
...
```{r}
FFL_C1_cont<- function(t, state, parameters) {
with(as.list(c(state, parameters)), { 
       # Acá van definidas las ecuaciones
       dX<- 0
       dY<- beta_y*hill_activator(X,0.5,2)-alfa_y*Y
       dZ <- beta_Z *hill_activator(X,0.5,2)*hill_activator(Y,0.5,2)-alfa_z*Z
        list(c( dX,dY, dZ))
      })
}
# Acá le damos valores a los parámetros
...
![](05_Images/Captura de Pantalla 2022-03-16 a la(s) 17.22.png)
# Introduction
