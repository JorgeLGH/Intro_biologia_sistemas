...
data_lung_survival <- read.table("SNFtool/Lung/LUNG_Survival.txt")

# Make them into matrices

data_lung_expression <- as.matrix(data_lung_expression)
data_lung_methylation <- as.matrix(data_lung_methylation)
data_lung_mirna <- as.matrix(data_lung_mirna)
data_lung_survival <- as.matrix(data_lung_survival)

# make smaller matrices for it to work, they must be the same dimensions, if not, it won't work

data_lung_expression <- data_lung_expression[1:nrow(data_lung_mirna),
                                             sample(ncol(data_lung_expression), ncol(data_lung_expression)/8)]

data_lung_methylation <- data_lung_methylation[1:nrow(data_lung_mirna),
                                               sample(ncol(data_lung_methylation), ncol(data_lung_methylation)/8)]

data_lung_mirna <- data_lung_mirna[,sample(ncol(data_lung_mirna), ncol(data_lung_mirna)/8)]


# make distance matrices

d1 <- dist2(data_lung_expression, data_lung_expression)
d2 <- dist2(data_lung_methylation, data_lung_methylation)
d3 <- dist2(data_lung_mirna, data_lung_mirna)
...
Fused <- SNF(list(af_m1, af_m2, af_m3), 20, 20)
